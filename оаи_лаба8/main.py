...
def create_summary_panel(
    ...
    draw.text((1008, 830), "В этой версии GLCM строится по всем 256 уровням яркости.", font=text_font, fill="#243447")
    draw.text((1008, 864), "Дополнительного уменьшения до 16 градаций больше нет.", font=text_font, fill="#243447")
    return panel
...
def build_readme_pdf(root: Path, rows: list[dict[str, object]]) -> None:
    ...
    y += 16
    y = section(draw1, "Что исправлено относительно прошлой версии", y)
    fixes = [
        "Дополнительное уменьшение яркости до 16 уровней полностью убрано.",
        "GLCM строится по полному диапазону 0…255, поэтому результаты соответствуют формулировке задания.",
        "Признак LUN вычисляется как локальная однородность Σ p(i,j)^2.",
        "Отчёт стал самодостаточным: в нём есть исходные изображения, полутоновые версии, контрастированные изображения, гистограммы, матрицы и численные выводы.",
    ]
    for fix in fixes:
    ...
    final_text = (
        ...
        "За счёт отказа от 16-уровневой квантизации матрицы GLCM и визуальные результаты теперь соответствуют заданию без лишней постеризации."
    )
